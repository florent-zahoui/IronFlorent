...
select * from bank.client where district_id = 1 order by client_id limit 5;
...
select account_id,amount from bank.loan where duration = 60 order by amount limit 5;
...
select count(client_id),district_id from bank.client where district_id < 10 group by district_id order by district_id;
# In the card table, how many cards exist for each type? Rank the result starting with the most frequent type.
select count(card_id),type from bank.card group by type order by count(card_id) desc;
# Using the loan table, print the top 10 account_ids based on the sum of all of their loan amounts.
select sum(amount),account_id from bank.loan group by account_id order by sum(amount) desc limit 10;
...
select count(loan_id),date,duration from bank.loan where date like "9712%" group by date,duration order by date,duration;
# In the trans table, for account_id 396, sum the amount of transactions for each type (VYDAJ = Outgoing, PRIJEM = Incoming). Your output should have the account_id, the type and the sum of amount, named as total_amount. Sort alphabetically by type.
select sum(amount) as total_amount,account_id,type from bank.trans where account_id = 396 group by type order by type;
...
SELECT floor(sum(amount)) AS total_amount,account_id,CASE WHEN type="PRIJEM" THEN "INCOMING" WHEN type="VYDAJ" THEN "OUTGOING" END AS type_name FROM bank.trans WHERE account_id = 396 GROUP BY type ORDER BY type;
